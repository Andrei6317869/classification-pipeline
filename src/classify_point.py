...
QUERY_RAW = QUERY * (bounds[1] - bounds[0]) + bounds[0]
...
def classify(values):
    ...
    return (
        None if values[best] < REJECT_TAU else classes[best],
        dist,
    )
...
for name, values in (
    ...
    print(
        f"{name:>9}: {verdict:>9} bytes="
        + " ".join(f"{v * 255:6.1f}" for v in values)
        + " dist="
        + " ".join(f"{d * 100:5.1f}%" for d in dist)
    )
...
print(
    f"\nquery {QUERY_RAW} -> cell [row {r}, col {c}] "
...
axg.set_title(
    f"Test query in cell (row {r}, col {c})",
    fontsize=9,
)
...
axp.set_title(
    f"argmax $\\rightarrow$ class {classes[winner]}",
    fontsize=9,
)
