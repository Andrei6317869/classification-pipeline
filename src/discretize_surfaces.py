...
print(
    f"{'storage':>9} {'B/cell':>7} {'per class':>10} "
    f"{'all ' + str(K):>8} {'vs uint8':>9}"
)

for name, width in FORMATS:
    print(
        f"{name:>9} {width:>7} {width * cells:>10} "
        f"{width * cells * K:>8} {width:>8}x"
    )
...
worst = max(
    np.abs(
        dequantize(quantize(surfaces[f"c{c}"]))
        - surfaces[f"c{c}"]
    ).max()
    for c in classes
)
...
fig.savefig(
    r"..\figures\generated\class_surfaces_discretized.pdf",
    bbox_inches="tight",
    transparent=True,
)