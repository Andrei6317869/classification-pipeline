...
surfaces = {
    c: normalise_peak(evaluate_on_grid(kdes[c], grid_points, xx))
    for c in classes
}
...
for ax, c in zip(np.atleast_1d(axes), classes):
    smooth = normalise_peak(
        np.exp(kdes[c].score_samples(fine)).reshape(fxx.shape)
    )
    im = ax.imshow(
        smooth,
        extent=[0, 1, 0, 1],
...
fig.savefig(
    r"..\figures\generated\class_surfaces_continuous.pdf",
    bbox_inches="tight",
    transparent=True,
)